...
def find_root_cause(exception):
    """ Return the deepest cause for a chained exception """
    seen = set()
    current = exception

    while current and id(current) not in seen:
        seen.add(id(current))
        next_exception = getattr(current, "__cause__", None)
        if next_exception is None:
            next_exception = getattr(current, "__context__", None)
        if next_exception is None:
            break
...
def format_request_exception(base_url, path, exception):
    full_url = "%s%s" % (base_url, path)
    root = find_root_cause(exception)

    if isinstance(exception, requests.exceptions.Timeout) or isinstance(
        root, (socket.timeout, TimeoutError)
    ):
        return (
            "[-] %s request failed: timeout. Check network connectivity or "
            "increase --timeout.\n"
        ) % full_url

    if isinstance(root, urllib3.exceptions.NameResolutionError) or isinstance(
        root, socket.gaierror
    ):
        return (
            "[-] %s request failed: could not resolve host. Check DNS or "
            "network connectivity.\n"
        ) % full_url

    if isinstance(exception, requests.exceptions.ConnectionError):
        return (
            "[-] %s request failed: connection error. Check network "
            "connectivity.\n"
        ) % full_url

    return "[-] %s request failed: %s\n" % (full_url, exception)
...
def fetch_git(url, directory, jobs, retry, timeout, http_headers, client_cert_p12=None, client_cert_p12_password=None):
    ...
    try:
        ...
        printf(
            format_request_exception(url, "/.git/HEAD", exc),
            file=sys.stderr,
        )
        return 1
    printf("[%d]\n", response.status_code)
    ...
    try:
        ...
        printf(
            format_request_exception(url, "/.git/", exc),
            file=sys.stderr,
        )
        return 1
    printf("[%d]\n", response.status_code)
...
def ensure_output_directory(path):
    """ Ensure output directory exists and is empty, or create a numeric suffix """
...
def derive_output_directory(url):
    """ Derive output directory name from URL host """
    parsed = urllib.parse.urlparse(url)
    if not parsed.scheme:
        parsed = urllib.parse.urlparse("http://%s" % url)
...
def main():
    ...
    if args.directory is None:
        args.directory = derive_output_directory(args.url)
        if args.directory is None:
            parser.error("output directory could not be derived from URL")

    output_directory, renamed = ensure_output_directory(args.directory)
    if output_directory is None:
        parser.error("`%s` is not a directory" % args.directory)
    if renamed:
        printf(
            "Warning: Destination '%s' is not empty, using '%s'\n",
            args.directory,
            output_directory,
        )
